...
- `ForestStartingStand` (on `Game State`) generates the stand deterministically at play start: a 19 x 19 planting lattice at 2 m spacing inside the borders, exactly 25 positions omitted (lowest FNV-hash slots - failed establishment/earlier mortality) leaving 336 living stems in visible rows with small deterministic planting jitter. Same-aged crop trees vary widely through a smooth deterministic vigour field (clusters of vigour and suppression, [D] bands 8-14 suppressed / 15-19 ordinary / 19-24 dominant, mean anchored at ~16 cm), with a release bonus on slots next to establishment gaps and extra suppression in tight interiors. Crowns initialise from `PotentialCrownRadiusM(DBH)`; the real systems then recompute Hegyi competition, crowns, canopy/light and wind - nothing hand-authored. IDs are stable `P{row}{col}` strings (never colliding with `R{year}-{cell}` recruits).
